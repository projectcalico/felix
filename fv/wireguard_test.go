...
var _ = infrastructure.DatastoreDescribe("WireGuard-Supported 3 node cluster", []apiconfig.DatastoreType{apiconfig.EtcdV3, apiconfig.Kubernetes}, func(getInfra infrastructure.InfraFactory) {
	...
	Context("with Wireguard disabled in node-3", func() {
		...
		It("transfer should be encrypted/plain between workloads on WireGuard enabled/disabled nodes", func() {
			...
			for i := range []int{0, 1} {
				Eventually(tcpdumpMatchCount(tcpdumps[i], "numTunnelPackets01"), "10s", "100ms").Should(BeNumerically(">", 0))
				Eventually(tcpdumpMatchCount(tcpdumps[i], "numTunnelPackets10"), "10s", "100ms").Should(BeNumerically(">", 0))
			}
			...
			for _, f := range []int{0, 2} {
				Eventually(tcpdumpMatchCount(tcpdumps[f], "numInWorkloadPackets"), "10s", "100ms").Should(BeNumerically(">", 0))
				Eventually(tcpdumpMatchCount(tcpdumps[f], "numOutWorkloadPackets"), "10s", "100ms").Should(BeNumerically(">", 0))
			}
		})
	})
})
