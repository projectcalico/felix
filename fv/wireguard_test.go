...
var _ = infrastructure.DatastoreDescribe("WireGuard-Supported", []apiconfig.DatastoreType{apiconfig.EtcdV3, apiconfig.Kubernetes}, func(getInfra infrastructure.InfraFactory) {
	...
	Context("traffic with Wireguard enabled", func() {
		...
		It("between pod to pod should be allowed and encrypted", func() {
			...
			for i := range felixes {
				Eventually(func() int {
					return tcpdumpMatchCount(tcpdumps[i], "numInTunnelPackets")
				}, "10s", "100ms").Should(BeNumerically(">", 0))
				Eventually(func() int {
					return tcpdumpMatchCount(tcpdumps[i], "numOutTunnelPackets")
				}, "10s", "100ms").Should(BeNumerically(">", 0))
				Eventually(func() int {
					return tcpdumpMatchCount(tcpdumps[i], "numWorkload01Packets")
				}, "10s", "100ms").Should(BeNumerically("==", 0))
				Eventually(func() int {
					return tcpdumpMatchCount(tcpdumps[i], "numWorkload10Packets")
				}, "10s", "100ms").Should(BeNumerically("==", 0))
			}
		})
