							...
							Describe("with policy enabling ingress to w[0][0] from host endpoints", func() {
								...
								It("should have connectivity from all host-networked workloads to workload 0", func() {
									...
									if testOpts.tunnel == "ipip" {
										hostW0IP = felixes[0].ExpectedIPIPTunnelAddr
										hostW1IP = felixes[1].ExpectedIPIPTunnelAddr
									}

									// Also try host networked pods, both on a local and remote node.
									// N.B. it cannot work without the connect time balancer
									cc.ExpectConnectivity(hostW[0], TargetIP(node0IP), []uint16{npPort}, ExpectWithSrcIPs(hostW0IP))
									cc.ExpectConnectivity(hostW[1], TargetIP(node0IP), []uint16{npPort}, ExpectWithSrcIPs(hostW1IP))
									cc.ExpectConnectivity(hostW[0], TargetIP(node1IP), []uint16{npPort}, ExpectWithSrcIPs(hostW0IP))
									cc.ExpectConnectivity(hostW[1], TargetIP(node1IP), []uint16{npPort}, ExpectWithSrcIPs(hostW1IP))
					...
					if !localOnly {
						It("should have connectivity from a workload via a nodeport on another node to workload 0", func() {
							...
							if testOpts.connTimeEnabled {
								// Also try host networked pods, both on a local and remote node.
								// N.B. it cannot work without the connect time balancer
								cc.ExpectSome(hostW[2], TargetIP(ip), npPort)
							}
							cc.CheckConnectivity()
