...
func (pc *PermanentConnection) Start() error {
...
func (pc *PermanentConnection) SinceLastPong() time.Duration {
	return time.Since(pc.LastPongTime())
}

func (pc *PermanentConnection) LastPongTime() time.Time {
	pc.Lock()
	defer pc.Unlock()
	return pc.lastPongTime
}

func (pc *PermanentConnection) PongCount() int {
	pc.Lock()
	defer pc.Unlock()
	return pc.pongCount
}
