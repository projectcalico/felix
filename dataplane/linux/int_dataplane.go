...
func NewIntDataplaneDriver(config Config) *InternalDataplane {
	...
	if config.RulesConfig.VXLANEnabled {
		routeTableVXLAN := routetable.New([]string{"^vxlan.calico$"}, 4, true, config.NetlinkTimeout,
			config.DeviceRouteSourceAddress, config.DeviceRouteProtocol, config.RouteProtocol, true, 0,
			dp.loopSummarizer)
	...
	routeTableV4 := routetable.New(interfaceRegexes, 4, false, config.NetlinkTimeout,
		config.DeviceRouteSourceAddress, config.DeviceRouteProtocol, config.RouteProtocol, config.RemoveExternalRoutes, 0,
		dp.loopSummarizer)
	...
	cryptoRouteTableWireguard := wireguard.New(config.Hostname, &config.Wireguard, config.NetlinkTimeout,
		config.DeviceRouteProtocol, config.RouteProtocol, func(publicKey wgtypes.Key) error {
	...
	if config.IPv6Enabled {
		...
		routeTableV6 := routetable.New(
			...
			config.DeviceRouteProtocol, config.RouteProtocol,
			config.RemoveExternalRoutes, 0,
			dp.loopSummarizer)
