...
func newBPFNeighManager(m bpf.Map) *bpfNeighManager {
	return &bpfNeighManager{
		arpMap:      m,
...
func (m *bpfNeighManager) OnUpdate(msg interface{}) {
	switch msg := msg.(type) {
	case *ifaceUpdate:
		m.onIfaceUpdate(msg)
	case *neighUpdate:
...
func (m *bpfNeighManager) onNeighUpdate(neigh *neighUpdate) {
	m.neighDirty = append(m.neighDirty, neigh)
}

func (m *bpfNeighManager) onIfaceUpdate(iface *ifaceUpdate) {
	m.ifaceDirty = append(m.ifaceDirty, iface)
}

func (m *bpfNeighManager) onRouteUpdate(update *proto.RouteUpdate) {
...
func (m *bpfNeighManager) onRouteRemove(update *proto.RouteRemove) {
	cidr := ip.MustParseCIDROrIP(update.Dst)
	v4CIDR, ok := cidr.(ip.V4CIDR)
	if !ok {
		// FIXME IPv6
...
func (m *bpfNeighManager) updateBPFMap(srcMAC net.HardwareAddr, n *neighUpdate) {
	k := arp.NewKey(n.IP, uint32(n.IfIndex))
	v := arp.NewValue(srcMAC, n.HWAddr)
	err := m.arpMap.Update(k[:], v[:])
	if err != nil {
		log.WithError(err).Warnf("Failed to update Neigh for IP %s dev %d", n.IP, n.IfIndex)
	} else {
		log.Debugf("Neigh updated for IP %s iface %d dstMAC %s", n.IP, n.IfIndex, n.HWAddr)
	}
}

func (m *bpfNeighManager) CompleteDeferredWork() error {
	...
	for _, n := range m.neighDirty {
		...
		if err != nil {
			log.WithError(err).Warnf("Failed to %s Neigh for IP %s dev %d", op, n.IP, n.IfIndex)
		} else {
			log.Debugf("Neigh %s for IP %s iface %d dstMAC %s", op, n.IP, n.IfIndex, n.HWAddr)
		}
	}
...
func (m *bpfNeighManager) keepUnresolved(neigh *neighUpdate) {
...
func (m *bpfNeighManager) removeUnresolved(neigh *neighUpdate) {
