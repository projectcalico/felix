...
var _ = Describe("RouteTable v6", func() {
	var dataplane *mocknetlink.MockNetlink
	var t *mocktime.MockTime
	var rt *RouteTable

	BeforeEach(func() {
		dataplane = mocknetlink.NewMockNetlink()
...
var _ = Describe("RouteTable", func() {
	...
	Describe("with some interfaces", func() {
		...
		for _, failFlags := range mocknetlink.RoutetableFailureScenarios {
			failFlags := failFlags
			...
			Describe(desc, func() {
				...
				JustBeforeEach(func() {
					...
					if failFlags == mocknetlink.FailNextLinkByName {
				...
				if failFlags&(mocknetlink.FailNextSetSocketTimeout|
					mocknetlink.FailNextNewNetlinkHandle|
