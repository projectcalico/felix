...
import (
	...
	"github.com/projectcalico/felix/shims/mock"
...
var _ = Describe("RouteTable v6", func() {
	var dataplane *mock.MockNetlink
	var t *mock.MockTime
	var rt *RouteTable

	BeforeEach(func() {
		dataplane = mock.NewMockNetlink()
		t = mock.NewMockTime()
	...
	It("should not remove the IPv6 link local route", func() {
		...
		Expect(dataplane.DeletedRouteKeys).ToNot(HaveKey(mock.KeyForRoute(&noopRoute)))
		Expect(dataplane.UpdatedRouteKeys).ToNot(HaveKey(mock.KeyForRoute(&noopRoute)))
	})
})
...
var _ = Describe("RouteTable", func() {
	...
	Describe("with some interfaces", func() {
		var cali1, cali3, eth0 *mock.MockLink
		...
		It("Should clear out a source address when source address is not set", func() {
			...
			err := rt.Apply()
			Expect(err).ToNot(HaveOccurred())
			Expect(dataplane.UpdatedRouteKeys).To(HaveKey(mock.KeyForRoute(&updateRoute)))
			Expect(dataplane.RouteKeyToRoute[mock.KeyForRoute(&updateRoute)]).To(Equal(fixedRoute))
		...
		Describe("With a device route source address set", func() {
			...
			It("Should delete routes without a source address", func() {
				err := rt.Apply()
				Expect(err).ToNot(HaveOccurred())
				Expect(dataplane.DeletedRouteKeys).To(HaveKey(mock.KeyForRoute(&cali3Route)))
				Expect(dataplane.DeletedRouteKeys).To(HaveKey(mock.KeyForRoute(&cali1Route)))
			})
		...
		Describe("after syncing, after adding a route and failing the update twice", func() {
			JustBeforeEach(func() {
				...
				dataplane.FailuresToSimulate = mock.FailNextRouteAdd
		...
		for _, failFlags := range mock.RoutetableFailureScenarios {
			failFlags := failFlags
			...
			Describe(desc, func() {
				...
				JustBeforeEach(func() {
					...
					if failFlags == mock.FailNextLinkByName {
				...
				It("should have consumed all failures", func() {
					...
					Expect(dataplane.FailuresToSimulate).To(Equal(mock.FailNone))
				})
				...
				if failFlags&(mock.FailNextSetSocketTimeout |
					mock.FailNextNewNetlinkHandle |
					mock.FailNextLinkByName |
					mock.FailNextLinkList |
					mock.FailNextRouteAdd |
					mock.FailNextRouteDel |
					mock.FailNextAddARP |
					mock.FailNextRouteList) != 0 {
	...
	Describe("with a down interface", func() {
		...
		for _, failure := range [] mock.FailFlags{
			mock.FailNextLinkByName,
			mock.FailNextRouteDel,
			mock.FailNextRouteList,
		} {
			failure := failure
