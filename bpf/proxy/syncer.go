...
func (s *Syncer) startupBuildPrev(state DPSyncerState) error {
	...
	// walk the frontend bpf map that was read into memory and match it agains the
	// references build off the state
	for svck, svcv := range s.origSvcs {
		...
		// created it - basedon the current state.
...
func (s *Syncer) startupSync(state DPSyncerState) error {
	...
	// crashed and apply only the diff using the regular code path.
