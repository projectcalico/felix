...
func (s *Syncer) activeBackends(ip net.IP, port uint16, proto uint8) map[ipPort]struct{} {
	id, ok := s.activeSvcsMap[ipPortProto{ipPort{ip.String(), int(port)}, proto}]
	if ok {
		return s.activeEpsMap[id]
	}
	return nil
}
	...
	return s.bpfCt.Iter(func(key, val []byte) {
		...
		case conntrack.TypeNATReverse:
			...
			if !s.frontendHasBackend(svcIP, svcPort, ipA, portA, proto) &&
				...
				log.WithField("key", k).Debugf("conntrackCleanup:TypeNATReverse deleted")
			} else {
				log.WithField("key", k).Debugf("conntrackCleanup:TypeNATReverse still active")
			}
		...
		case conntrack.TypeNATForward:
			...
			if !s.frontendHasBackend(svcIP, svcPort, epIP, epPort, proto) {
				...
				log.WithField("key", k).Debugf("conntrackCleanup:TypeNATForward deleted")
			} else {
				log.WithField("key", k).Debugf("conntrackCleanup:TypeNATForward still active")
			}
...
func epInEps(ep k8sp.Endpoint, eps []k8sp.Endpoint) bool {
	for _, x := range eps {
		// XXX possibly benign changes like IsLocal()=true vs undefined or vise
		// versa are reflected here.
		if ep.Equal(x) {
			return true
		}
	}
