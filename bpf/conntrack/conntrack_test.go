...
var _ = Describe("BPF Conntrack StaleNATScanner", func() {
	...
	DescribeTable("forward entries",
		...
		Entry("keyA - revA",
			...
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, svcIP, svcPort, backendIP, backendPort)),
			conntrack.ScanVerdictDelete,
		),
		Entry("keyA - revB",
			conntrack.NewKey(123, clientIP, clientPort, svcIP, svcPort),
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, backendIP, backendPort, svcIP, svcPort)),
			conntrack.ScanVerdictDelete,
		),
		Entry("keyB - revA",
		...
		Entry("mismatch key port",
			conntrack.NewKey(123, svcIP, 54545, clientIP, clientPort),
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, backendIP, backendPort, svcIP, svcPort)),
			conntrack.ScanVerdictOK,
		),
		Entry("mismatch key IP",
			conntrack.NewKey(123, net.IPv4(2, 1, 2, 1), svcPort, clientIP, clientPort),
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, backendIP, backendPort, svcIP, svcPort)),
			conntrack.ScanVerdictOK,
		),
		Entry("mismatch rev port",
			conntrack.NewKey(123, svcIP, svcPort, clientIP, clientPort),
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, backendIP, backendPort, svcIP, 12321)),
			conntrack.ScanVerdictOK,
		),
		Entry("mismatch rev IP",
			conntrack.NewKey(123, svcIP, svcPort, clientIP, clientPort),
			conntrack.NewValueNATForward(0, 0, 0, conntrack.NewKey(123, backendIP, backendPort, net.IPv4(3, 2, 2, 3), svcPort)),
			conntrack.ScanVerdictOK,
		),
	)
})
