...
func (x bpfSyscallMapMemCtx) GetMapNextKey(mapFD MapFD, k []byte, keySize int) ([]byte, error) {
	...
	errno := C.bpf_map_call(unix.BPF_MAP_GET_NEXT_KEY, C.uint(mapFD), cK, x.cK, 0)
	if errno != 0 {
		return nil, unix.Errno(errno)
	}

	return C.GoBytes(x.cK, C.int(keySize)), nil
}

func (x bpfSyscallMapMemCtx) GetMapEntry(mapFD MapFD, k []byte, valueSize int) ([]byte, error) {
	cK := unsafe.Pointer(&k[0])

	errno := C.bpf_map_call(unix.BPF_MAP_LOOKUP_ELEM, C.uint(mapFD), cK, x.cV, unix.BPF_ANY)
	if errno != 0 {
		return nil, unix.Errno(errno)
	}

	return C.GoBytes(x.cV, C.int(valueSize)), nil
}
