...
func NewWithShims(
	...
	logrus.WithFields(logrus.Fields{"enabled": config.Enabled, "wgIfaceName": config.InterfaceName}).Info(
		"Initialised wireguard dataplane driver.")
...
func (w *Wireguard) OnIfaceStateChanged(ifaceName string, state ifacemonitor.State) {
	if w.config.InterfaceName != ifaceName {
		logrus.WithField("ifaceName", ifaceName).Debug("Ignoring interface state change, not the wireguard interface.")
		return
	}
	switch state {
	case ifacemonitor.StateUp:
		logrus.Debug("Interface up, marking for route sync")
		if !w.ifaceUp {
			w.ifaceUp = true
			w.inSyncWireguard = false
		}
	case ifacemonitor.StateDown:
		logrus.Debug("Interface down")
		w.ifaceUp = false
	}
...
func (w *Wireguard) EndpointUpdate(name string, ipv4Addr ip.Addr) {
	logrus.Debugf("EndpointUpdate: name=%s; ipv4Addr=%v", name, ipv4Addr)
	if !w.config.Enabled {
		logrus.Debug("Not enabled - ignoring")
		return
	...
	if existing, ok := w.peers[name]; ok && existing.ipv4EndpointAddr == ipv4Addr {
		logrus.Debug("Update contains unchanged IPv4 address")
		update.ipv4EndpointAddr = nil
	} else {
		logrus.Debug("Update contains new IPv4 address")
...
func (w *Wireguard) EndpointRemove(name string) {
	logrus.Debugf("EndpointRemove: name=%s", name)
	if !w.config.Enabled {
		logrus.Debug("Not enabled - ignoring")
		return
	} else if name == w.hostname {
		logrus.Debug("Local update - ignoring")
		return
	}
...
func (w *Wireguard) EndpointAllowedCIDRAdd(name string, cidr ip.CIDR) {
	...
	if existing, ok := w.peers[name]; ok && existing.cidrs.Contains(cidr) {
		...
		logrus.Debug("Node CIDR added which is already programmed - remove any pending delete")
		...
		logrus.Debug("Node CIDR added which is not programmed")
...
func (w *Wireguard) EndpointAllowedCIDRRemove(cidr ip.CIDR) {
	logrus.Debugf("EndpointAllowedCIDRRemove: cidr=%v", cidr)
	if !w.config.Enabled {
		logrus.Debug("Not enabled - ignoring")
		return
	}
	...
	if !ok {
		logrus.Debugf("CIDR not found as node update, checking current configuration")
		...
		if !ok {
			...
			logrus.Debugf("CIDR remove update but not associated with a node: %v", cidr)
			return
		}
	}
	logrus.Debugf("CIDR found for node %s", name)

	update := w.getOrInitPeerUpdate(name)
	if existing, ok := w.peers[name]; ok && existing.cidrs.Contains(cidr) {
		// Remove the CIDR from a node that already has the CIDR configured.
		logrus.Debug("Node CIDR removed")
		...
		logrus.Debug("Node CIDR removed but is not programmed - remove any pending add")
...
func (w *Wireguard) EndpointWireguardUpdate(name string, publicKey wgtypes.Key, ipv4InterfaceAddr ip.Addr) {
	logrus.Debugf("EndpointWireguardUpdate: name=%s; key=%s, ipv4Addr=%v", name, publicKey, ipv4InterfaceAddr)
	if !w.config.Enabled {
		logrus.Debug("Not enabled - ignoring")
		return
	}
	...
	if name == w.hostname {
		logrus.Debug("Local wireguard info updated")
		if w.ourPublicKey == nil || *w.ourPublicKey != publicKey {
			...
			// and publish.
			logrus.Debug("Stored public key does not match key queried from dataplane")
			w.ourPublicKeyAgreesWithDataplaneMsg = false
		}
		if w.ourIPv4InterfaceAddr != ipv4InterfaceAddr {
			logrus.Debug("Local interface addr updated")
	...
	if existing, ok := w.peers[name]; ok && existing.publicKey == publicKey {
		...
		logrus.Debug("Public key unchanged from programmed")
		update.publicKey = nil
	} else {
		// Public key updated (or this is a previously unseen node)
		logrus.Debug("Storing updated public key")
...
func (w *Wireguard) EndpointWireguardRemove(name string) {
	logrus.Debugf("EndpointWireguardRemove: name=%s", name)
	if !w.config.Enabled {
		logrus.Debug("Not enabled - ignoring")
		return
	}
	...
	if _, ok := w.peers[name]; ok {
		logrus.Debugf("Peer %s is programmed", name)
	} else if _, ok := w.peerUpdates[name]; !ok {
		logrus.Debugf("Peer %s is not programmed, and there are no updates", name)
		return
	}
...
func (w *Wireguard) QueueResync() {
	logrus.Info("Queueing a resync of wireguard configuration")
...
func (w *Wireguard) Apply() (err error) {
	...
	defer func() {
		...
		if !w.ourPublicKeyAgreesWithDataplaneMsg && w.ourPublicKey != nil {
			logrus.Infof("Public key out of sync or updated: %s", *w.ourPublicKey)
	...
	if err != nil {
		logrus.Errorf("error obtaining link client: %v", err)
		return err
	}

	// If wireguard is not enabled, then short-circuit the processing - ensure config is deleted.
	if !w.config.Enabled {
		logrus.Info("Wireguard is not enabled")
		if !w.inSyncWireguard {
			logrus.Debug("Wireguard is not in-sync - verifying wireguard configuration is removed")
	...
	if w.wireguardNotSupported {
		logrus.Info("Wireguard is not supported")
		return
	}
		...
		if len(w.peerUpdates) > 0 {
			for name, node := range w.peers {
				if w.shouldProgramWireguardPeer(name, node) {
					logrus.Debugf("Flag node %s as programmed", name)
					node.programmedInWireguard = true
				} else {
					logrus.Debugf("Flag node %s as not programmed", name)
	...
	// doing anything else.
	if !w.inSyncLink {
		logrus.Debug("Ensure wireguard link is created and up")
		...
		if netlinkshim.IsNotSupported(err) {
			...
			logrus.Info("Wireguard is not supported - publishing no public key")
			w.setNotSupported()
			return nil
		} else if err != nil {
			// Error configuring link, pass up the stack. Close the netlink client as a precaution.
			logrus.WithError(err).Info("Unable to create wireguard link, retrying...")
			w.closeNetlinkClient()
			return ErrUpdateFailed
		} else if !linkUp {
			// Wait for oper up notification.
			logrus.Info("Waiting for wireguard link to come up...")
			return nil
		}
	}
	...
	if netlinkshim.IsNotSupported(err) {
		logrus.Info("Wireguard is not supported - send zero-key status")
		w.setNotSupported()
		return nil
	} else if err != nil {
		logrus.WithError(err).Error("error obtaining wireguard client")
		return ErrUpdateFailed
	}
	...
	if !w.inSyncInterfaceAddr {
		logrus.Info("Ensure wireguard interface address is correct")
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	logrus.Debug("Apply routing table updates for wireguard")
	wg.Add(1)
	go func() {
		defer wg.Done()
	...
	go func() {
		...
		if w.inSyncWireguard {
			...
			logrus.Debug("Apply wireguard crypto routing delta update")
			if errWireguard = w.applyWireguardConfig(wireguardClient, wireguardPeerDelete); errWireguard != nil {
				logrus.WithError(errWireguard).Info("Failed to delete wireguard peers")
				return
			}
			...
			if errWireguard = w.applyWireguardConfig(wireguardClient, wireguardPeerUpdate); errWireguard != nil {
				logrus.WithError(errWireguard).Info("Failed to create or update wireguard peers")
				return
			}
		} else {
			...
			logrus.Debug("Apply wireguard crypto routing resync")
			if publicKey, wireguardPeerUpdate, errWireguard = w.constructWireguardDeltaForResync(wireguardClient); errWireguard != nil {
				logrus.WithError(errWireguard).Info("Failed to construct a full wireguard delta for resync")
				return
			} else if errWireguard = w.applyWireguardConfig(wireguardClient, wireguardPeerUpdate); errWireguard != nil {
				logrus.WithError(errWireguard).Info("Failed to update wireguard peers for resync")
				return
				...
				logrus.Infof("Public key has been updated to %s, send status notification", publicKey)
	...
	if errWireguard != nil {
		...
		logrus.Info("Wireguard programming failed, ensure full resync is performed next")
	...
	// routing table.
	logrus.Debug("Ensure routing rule is configured")
...
func (w *Wireguard) handlePeerAndRouteDeletionFromPeerUpdates(conflictingKeys set.Set) *wgtypes.Config {
	...
	for name, update := range w.peerUpdates {
		...
		logrus.Debugf("Handle peer and route deletion for node %s", name)
		node := w.peers[name]
		if node == nil {
			logrus.Debugf("No wireguard configuration for node %s", name)
			continue
		}

		if update.deleted {
			// Node is deleted, so remove the node configuration and the associated routes.
			logrus.Infof("Node %s is deleted, remove associated routes and wireguard peer", name)
			delete(w.peers, name)
			...
			logrus.Debugf("Node %s updated, but public key is the same, no wireguard peer deletion required", name)
			continue
		}

		if node.publicKey == zeroKey {
			// The node did not have a key assigned, so no peer tidy-up required.
			logrus.Debugf("Node %s had no public key assigned, so no deletion of wireguard peer necessary", name)
			continue
		}
		...
		if node.programmedInWireguard && w.inSyncWireguard {
			logrus.Debugf("Adding peer deletion config update for key %s", node.publicKey)
		...
		if nodenames.Len() == 0 {
			...
			logrus.Debugf("Removed the only node %s claiming public key %s", name, node.publicKey)
			...
			logrus.Infof("Removed node %s which claimed the same public key %s to at least one other node", name, node.publicKey)
	...
	if len(wireguardPeerDelete.Peers) > 0 {
		logrus.Debug("There are wireguard peers to delete")
...
func (w *Wireguard) updateCacheFromPeerUpdates(conflictingKeys set.Set) {
	for name, update := range w.peerUpdates {
		...
		logrus.Debugf("Updating cache from update for peer %s", name)
		updated := false
		if update.ipv4EndpointAddr != nil {
			logrus.Debugf("Store IPv4 address %s", *update.ipv4EndpointAddr)
			node.ipv4EndpointAddr = *update.ipv4EndpointAddr
			updated = true
		}
		if update.publicKey != nil {
			logrus.Debugf("Store public key %s", *update.publicKey)
			node.publicKey = *update.publicKey
			if node.publicKey != zeroKey {
				if nodenames := w.publicKeyToNodeNames[node.publicKey]; nodenames == nil {
					logrus.Debug("Public key not associated with a node")
					w.publicKeyToNodeNames[node.publicKey] = set.From(name)
				} else {
					logrus.Info("Public key already associated with a node")
					conflictingKeys.Add(node.publicKey)
					nodenames.Add(name)
				}
			}
		...
		update.allowedCidrsDeleted.Iter(func(item interface{}) error {
			cidr := item.(ip.CIDR)
			logrus.Debugf("Discarding CIDR %s", cidr)
		...
		update.allowedCidrsAdded.Iter(func(item interface{}) error {
			cidr := item.(ip.CIDR)
			logrus.Debugf("Adding CIDR %s", cidr)
			node.cidrs.Add(cidr)
		...
		if updated {
			...
			logrus.Debug("Node updated")
			w.setPeer(name, node)
		} else {
			// No further update, delete update so it's not processed again.
			logrus.Debug("No updates for the node - remove node update to remove additional processing")
			delete(w.peerUpdates, name)
		}
	}
}
...
func (w *Wireguard) updateRouteTableFromPeerUpdates() {
	...
	for name, update := range w.peerUpdates {
		...
		update.allowedCidrsDeleted.Iter(func(item interface{}) error {
			logrus.Debugf("Removing CIDR %s (node %s) from routetable interface %s", item, name, ifaceName)
			cidr := item.(ip.CIDR)
	...
	for name, update := range w.peerUpdates {
		logrus.Debugf("Add/update routing for peer %s", name)
		node := w.getOrInitPeer(name)
		...
		if node.routingToWireguard != shouldRouteToWireguard {
			logrus.Debugf("Wireguard routing has changed from %v to %v - need to update full set of CIDRs", node.routingToWireguard, shouldRouteToWireguard)
			updateSet = node.cidrs
		} else {
			logrus.Debugf("Wireguard routing has not changed from %v - only need to update added CIDRs", node.routingToWireguard)
		...
		if !shouldRouteToWireguard {
			...
			logrus.Debug("Not routing to wireguard - set route type to throw")
			...
			logrus.Debug("Routing to wireguard interface")
			ifaceName = w.config.InterfaceName
		...
		updateSet.Iter(func(item interface{}) error {
			cidr := item.(ip.CIDR)
			logrus.Debugf("Updating route for CIDR %s", cidr)
			if node.routingToWireguard != shouldRouteToWireguard {
				...
				logrus.Debugf("Wireguard routing has changed - delete previous route for %s", deleteIfaceName)
...
func (w *Wireguard) constructWireguardDeltaFromPeerUpdates(conflictingKeys set.Set) *wgtypes.Config {
	...
	if w.inSyncWireguard {
		...
		for name, update := range w.peerUpdates {
			logCxt := logrus.WithField("peer", name)
			logCxt.Debug("Constructing wireguard delta")
			peer := w.peers[name]
			if peer == nil {
				logrus.Warning("internal error: peer data is nil")
				continue
			}
		...
		conflictingKeys.Iter(func(item interface{}) error {
			logrus.Debugf("Processing public key with conflicting peers: %s", item)
			...
			nodenames.Iter(func(item interface{}) error {
				...
				logrus.Debugf("Processing peer %s", nodename)
				...
				if peer == nil || peer.programmedInWireguard == w.shouldProgramWireguardPeer(nodename, peer) {
					...
					logrus.Debug("Programming state has not changed")
					return nil
				} else if peer.programmedInWireguard {
					// The peer is programmed and shouldn't be. Add a delta delete.
					logrus.Debug("Programmed in wireguard, need to delete")
					wireguardUpdate.Peers = append(wireguardUpdate.Peers, wgtypes.PeerConfig{
						Remove:    true,
						PublicKey: peer.publicKey,
					})
					...
					logrus.Debug("Not programmed in wireguard, needs to be added now")
					wireguardUpdate.Peers = append(wireguardUpdate.Peers, wgtypes.PeerConfig{
						PublicKey:  peer.publicKey,
	...
	if len(wireguardUpdate.Peers) > 0 {
		logrus.Debug("There are peers to update")
...
func (w *Wireguard) constructWireguardDeltaForResync(wireguardClient netlinkshim.Wireguard) (wgtypes.Key, *wgtypes.Config, error) {
	...
	if err != nil {
		logrus.Errorf("error querying wireguard configuration: %v", err)
	...
	if device.FirewallMark != w.config.FirewallMark {
		logrus.Infof("Update firewall mark from %d to %d", device.FirewallMark, w.config.FirewallMark)
	...
	if device.ListenPort != w.config.ListeningPort {
		logrus.Infof("Update listening port from %d to %d", device.ListenPort, w.config.ListeningPort)
	...
	if device.PrivateKey == zeroKey || device.PublicKey == zeroKey {
		...
		// public key.
		logrus.Info("Generate new private/public keypair")
	...
	for peerIdx := range device.Peers {
		...
		if node == nil {
			logrus.Infof("Peer key is not expected or associated with multiple peers: %v", key)
			wireguardUpdate.Peers = append(wireguardUpdate.Peers, wgtypes.PeerConfig{
				PublicKey: key,
				Remove:    true,
			})
		...
		logrus.Debugf("Checking allowed CIDRs for node with key %v", key)
		...
		logrus.Debug("Check programmed CIDRs for required deletions")
		for _, netCidr := range configuredCidrs {
			...
			if !node.cidrs.Contains(cidr) {
				...
				logrus.Debugf("Unexpected CIDR configured: %s", cidr)
				replaceCidrs = true
				break
			}
		}
		...
		if replaceCidrs || replaceEndpointAddr {
			...
			if replaceEndpointAddr {
				logrus.Info("Endpoint address needs updating")
				peer.Endpoint = w.endpointUDPAddr(expectedEndpointIP)
			}

			if replaceCidrs {
				logrus.Info("AllowedIPs need replacing")
	...
	for name, node := range w.peers {
		if processedKeys.Contains(node.publicKey) {
			logrus.Debugf("Peer key already handled: node %s; key %v", name, node.publicKey)
			continue
		}
		if !w.shouldProgramWireguardPeer(name, node) {
			logrus.Debugf("Peer should not be programmed: node %s", name)
			continue
		}

		logrus.Infof("Add peer to wireguard: node %s; key %v; ip: %v", name, node.publicKey, node.ipv4EndpointAddr)
		wireguardUpdate.Peers = append(wireguardUpdate.Peers, wgtypes.PeerConfig{
			PublicKey:  node.publicKey,
	...
	logrus.Debugf("Wireguard device configured with public key %v", publicKey)
...
func (w *Wireguard) ensureLink(netlinkClient netlinkshim.Netlink) (bool, error) {
	...
	if netlinkshim.IsNotExist(err) {
		...
		logrus.Info("Wireguard device needs to be created")
		...
		link, err = netlinkClient.LinkByName(w.config.InterfaceName)
		if err != nil {
			logrus.Errorf("error querying wireguard device: %v", err)
			return false, err
		}

		logrus.Info("Created wireguard device")
	} else if err != nil {
		logrus.Errorf("unable to determine if wireguard device exists: %v", err)
		return false, err
	}

	if link.Type() != wireguardType {
		logrus.Errorf("interface %s is of type %s, not wireguard", w.config.InterfaceName, link.Type())
		return false, errWrongInterfaceType
	}

	// If necessary, update the MTU and admin status of the device.
	logrus.Debug("Wireguard device exists, checking settings")
	attrs := link.Attrs()
	oldMTU := attrs.MTU
	if w.config.MTU != 0 && oldMTU != w.config.MTU {
		logrus.WithField("oldMTU", oldMTU).Info("Wireguard device MTU needs to be updated")
		if err := netlinkClient.LinkSetMTU(link, w.config.MTU); err != nil {
			logrus.WithError(err).Warn("failed to set tunnel device MTU")
			return false, err
		}
		logrus.Info("Updated wireguard device MTU")
	}
	if attrs.Flags&net.FlagUp == 0 {
		logrus.WithField("flags", attrs.Flags).Info("Wireguard interface wasn't admin up, enabling it")
		if err := netlinkClient.LinkSetUp(link); err != nil {
			logrus.WithError(err).Warn("failed to set wireguard device up")
			return false, err
		}
		logrus.Info("Set wireguard admin up")

		if link, err = netlinkClient.LinkByName(w.config.InterfaceName); err != nil {
			logrus.WithError(err).Warn("failed to get link device after creating link")
			return false, err
		}
	}
...
func (w *Wireguard) ensureNoLink(netlinkClient netlinkshim.Netlink) error {
	...
	if err == nil {
		...
		logrus.Info("Wireguard is disabled, deleting device")
		if err := netlinkClient.LinkDel(link); err != nil {
			logrus.Errorf("error deleting wireguard type link: %v", err)
			return err
		}
		logrus.Info("Deleted wireguard device")
	} else if netlinkshim.IsNotExist(err) {
		logrus.Debug("Wireguard is disabled and does not exist")
	} else if err != nil {
		logrus.Errorf("unable to determine if wireguard device exists: %v", err)
		return err
	}
...
func (w *Wireguard) ensureLinkAddressV4(netlinkClient netlinkshim.Netlink) error {
	logrus.Debug("Setting local IPv4 address on link.")
	link, err := netlinkClient.LinkByName(w.config.InterfaceName)
	if err != nil {
		logrus.WithError(err).Warning("Failed to get device")
		return err
	}
	...
	for _, oldAddr := range addrs {
		if address != nil && oldAddr.IP.Equal(address) {
			logrus.Debug("Address already present.")
			found = true
			continue
		}
		logrus.WithField("oldAddr", oldAddr).Info("Removing old address")
		if err := netlinkClient.AddrDel(link, &oldAddr); err != nil {
			logrus.WithError(err).Warn("failed to delete address from wireguard device")
			return err
		}
	}

	if !found && address != nil {
		logrus.Info("address not present on wireguard device, adding it")
...
func (w *Wireguard) ensureRouteRule(netlinkClient netlinkshim.Netlink) error {
	...
	for _, rule := range rules {
		if rule.Table == w.config.RoutingTableIndex {
			logrus.Debugf("Found rule to table %d", w.config.RoutingTableIndex)
			if reflect.DeepEqual(rule, *newrule) {
				logrus.Debugf("Rule matches required rule")
				found = true
				continue
			}
			...
			if err := netlinkClient.RuleDel(&rule); err != nil {
				logrus.WithError(err).Error("Unable to delete wireguard routing rule")
				return err
			}
		}
	}
	...
	if err := netlinkClient.RuleAdd(newrule); err != nil {
		logrus.WithError(err).Error("Unable to create wireguard routing rule")
		return err
	} else {
		logrus.Debugf("Added rule: %#v", newrule)
	}
	...
	for _, rule := range rules {
		if rule.Table == w.config.RoutingTableIndex {
			...
			if err := netlinkClient.RuleDel(&rule); netlinkshim.IsNotExist(err) {
				logrus.Debug("Wireguard routing rule already deleted")
			} else if err != nil {
				logrus.WithError(err).Error("Unable to delete wireguard routing rule")
				return err
			}
...
func (w *Wireguard) shouldProgramWireguardPeer(name string, node *peerData) bool {
	if node.ipv4EndpointAddr == nil {
		logrus.Debugf("Peer %s should not be programmed, no endpoint address", name)
		return false
	} else if node.publicKey == zeroKey {
		logrus.Debugf("Peer %s should not be programmed, no valid public key", name)
		return false
	} else if w.publicKeyToNodeNames[node.publicKey].Len() != 1 {
		logrus.Debugf("Peer %s should not be programmed, multiple nodes are claiming the same key", name)
		return false
	}
	logrus.Debugf("Peer %s should be programmed", name)
	return true
}
...
func (w *Wireguard) getWireguardClient() (netlinkshim.Wireguard, error) {
	if w.cachedWireguardClient == nil {
		if w.numConsistentWireguardClientFailures >= maxConnFailures && w.numConsistentWireguardClientFailures%wireguardClientRetryInterval != 0 {
			...
			logrus.WithField("numFailures", w.numConsistentWireguardClientFailures).Debug(
				"Repeatedly failed to connect to wireguard client.")
			return nil, ErrNotSupportedTooManyFailures
		}
		logrus.Info("Trying to connect to wireguard client")
		client, err := w.newWireguardClient()
		if err != nil {
			w.numConsistentWireguardClientFailures++
			logrus.WithError(err).WithField("numFailures", w.numConsistentWireguardClientFailures).Info(
	...
	if w.numConsistentWireguardClientFailures > 0 {
		logrus.WithField("numFailures", w.numConsistentWireguardClientFailures).Info(
...
func (w *Wireguard) closeWireguardClient() {
	...
	if err := w.cachedWireguardClient.Close(); err != nil {
		logrus.WithError(err).Error("Failed to close wireguard client, ignoring.")
	}
...
func (w *Wireguard) getNetlinkClient() (netlinkshim.Netlink, error) {
	if w.cachedNetlinkClient == nil {
		...
		if w.numConsistentNetlinkClientFailures >= maxConnFailures {
			logrus.WithField("numFailures", w.numConsistentNetlinkClientFailures).Panic(
				"Repeatedly failed to connect to netlink.")
		}
		logrus.Info("Trying to connect to linkClient")
		client, err := w.newNetlinkClient()
		if err != nil {
			w.numConsistentNetlinkClientFailures++
			logrus.WithError(err).WithField("numFailures", w.numConsistentNetlinkClientFailures).Error(
	...
	if w.numConsistentNetlinkClientFailures > 0 {
		logrus.WithField("numFailures", w.numConsistentNetlinkClientFailures).Info(
			"Connected to linkClient after previous failures.")
...
func (w *Wireguard) applyWireguardConfig(wireguardClient netlinkshim.Wireguard, c *wgtypes.Config) error {
	logrus.Debugf("Apply wireguard config update: %#v", c)
	if c == nil {
		// No config to apply.
		return nil
	}
