...
class DevicesPlugin(FelixPlugin):
    ...
    def configure_interface_ipv4(self, if_name):
        """
        Configure the host side of the given interface for Calico usage.

        For example,
          - Allow packets from controlled interfaces to be directed to
            localhost
          - Enable proxy ARP
    ...
    def interface_watcher(self, update_splitter):
        """Create a new interface watcher actor."""
        pass


class LinuxKernelDevices(DevicesPlugin):
    def do_global_configuration(self):
        """
        Configures the global kernel config.  In particular, sets the flags
        that we rely on to ensure security, such as the kernel's RPF check.
